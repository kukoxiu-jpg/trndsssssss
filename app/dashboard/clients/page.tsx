...
import type { Client, Payment } from "@/lib/db";
...
export default function ClientsPage() {
  ...
  const load = () => setClients(getAll<Client>("clients"));
  useEffect(() => { load(); }, []);

  const openNew = () => { setEditing(null); setForm(emptyForm); setModalOpen(true); };
  const openEdit = (c: Client) => { setEditing(c); setForm({ name: c.name, phone: c.phone || "", address: c.address || "", balance: c.balance }); setModalOpen(true); };

  const save = () => {
    if (!form.name.trim()) return;
    if (editing) update<Client>("clients", editing.id, form);
    else add<Client>("clients", form as any);
    setModalOpen(false);
    load();
  };

  const del = (id: string) => {
    if (confirm(isAr ? "هل تريد الحذف؟" : "Confirmer la suppression ?")) { remove("clients", id); load(); }
  };

  const recordPayment = () => {
    if (!payModal || payAmount <= 0) return;
    add<Payment>("payments", { entityType: "client", entityId: payModal.id, amount: payAmount, method: payMethod, date: new Date().toISOString().slice(0, 10) });
    update<Client>("clients", payModal.id, { balance: Math.max(0, payModal.balance - payAmount) });
  ...
  const label = {
    ...
    add: isAr ? "إضافة عميل" : "Ajouter un client",
    ...
    balance: isAr ? "الرصيد" : "Solde", actions: isAr ? "إجراءات" : "Actions",
    ...
    edit: isAr ? "تعديل العميل" : "Modifier le client",
  };
          ...
          <tbody>
            {filtered.map((c) => (
              <tr key={c.id} className="border-t border-gray-50 hover:bg-gray-50/50">
                ...
                <td className={`px-4 py-3 text-right font-semibold ${c.balance > 0 ? "text-accent" : "text-emerald-600"}`}>{formatCurrency(c.balance)}</td>
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-2">
                    <button title={label.payment} onClick={() => { setPayModal(c); setPayAmount(0); }} className="text-primary hover:bg-primary-light p-1.5 rounded"><Wallet size={16} /></button>
                    <button onClick={() => openEdit(c)} className="text-gray-500 hover:bg-gray-100 p-1.5 rounded"><Pencil size={16} /></button>
                    <button onClick={() => del(c.id)} className="text-gray-400 hover:bg-gray-100 p-1.5 rounded"><Trash2 size={16} /></button>
                  </div>
                </td>
              </tr>
            ))}
            {filtered.length === 0 && <tr><td colSpan={5} className="text-center text-gray-400 py-8">{isAr ? "لا توجد نتائج" : "Aucun client trouvé"}</td></tr>}
          </tbody>
        </table>
      </div>
